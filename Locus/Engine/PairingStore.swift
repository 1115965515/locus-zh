...
final class PairingStore: ObservableObject {
    ...
    func refresh() {
        ...
        if FileManager.default.fileExists(atPath: pairingURL.path),
           ...
           looksLikePairingPlist(data) {
            if PairingKeychain.load() == nil {
                PairingKeychain.save(data)
            }
        } else if let data = PairingKeychain.load(), looksLikePairingPlist(data) {
...
/// Survives uninstall when the new install is signed by the same team and bundle ID.
private enum PairingKeychain {
    static let service = "com.chrismack.locus.rppairing"
    static let account = "rp_pairing_file"
...
enum PairingImportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .emptyClipboard:
            return "剪贴板为空。请复制 RPPairing plist 文本（或文件）后再粘贴。"
        case .invalidContents:
            return "这不像 RPPairing plist。请复制完整配对文件内容后重试。"
        }
    }
}
