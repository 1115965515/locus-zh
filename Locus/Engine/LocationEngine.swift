...
enum LocationEngineError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidIP: return "隧道 IP 无效。请在设置 → 隧道 IP 中检查（通常为 10.7.0.1）。"
        case .pairingRead: return "无法读取 RPPairing 文件。请用 idevice_pair 的 RPPairing 模式生成。"
        case .tunnelCreate: return "无法打开开发者隧道。LocalDevVPN 是否已在 Wi‑Fi 下连接？"
        case .remoteServer: return "已连上隧道，但 RemoteXPC 握手失败。"
        case .simulationCreate: return "无法打开 Apple 的位置模拟服务。"
        case .locationSet: return "设置模拟坐标失败。"
        case .locationClear: return "清除模拟位置失败。"
...
enum LocationEngine {
    ...
    static func set(latitude: Double, longitude: Double, pairingPath: String, deviceIP: String) -> Result<Void, LocationEngineError> {
        // MapKit picks in mainland China are GCJ-02; locationd expects WGS-84.
        let inject = ChinaCoordinate.gcj02ToWgs84Exact(
            CLLocationCoordinate2D(latitude: latitude, longitude: longitude)
        )
        var result: Result<Void, LocationEngineError> = .failure(.locationSet)
        queue.sync {
            let code = setLocked(
                latitude: inject.latitude,
                longitude: inject.longitude,
