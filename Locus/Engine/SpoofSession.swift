...
final class SpoofSession: ObservableObject {
    ...
    @Published var pinAddressLoading = false
    @Published var pinPlaceName: String?
    ...
    private static func isGenericFavoriteName(_ name: String) -> Bool {
        let trimmed = name.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.isEmpty || trimmed == "收藏" || trimmed == "Favorite" { return true }
    ...
    private func pushRecent(_ coordinate: CLLocationCoordinate2D) {
        let name = suggestedFavoriteName(
            for: coordinate,
            fallback: pinPlaceName ?? pinAddress
        )
        pushNamedRecent(name: name, coordinate: coordinate)
    }
    ...
    /// Upgrade a recent entry that was saved with only coordinates once an address is known.
    func updateRecentName(for coordinate: CLLocationCoordinate2D, name: String) {
        let trimmed = name.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        guard let index = recents.firstIndex(where: {
            abs($0.latitude - coordinate.latitude) < 0.00015 &&
            abs($0.longitude - coordinate.longitude) < 0.00015
        }) else { return }
        if Self.isGenericFavoriteName(recents[index].name) {
            recents[index].name = trimmed
            SavedPlace.save(recents, key: recentsKey)
        }
    }
