...
/// China map offset: MapKit in Greater China (mainland, HK, MO, TW) uses GCJ-02
/// (火星坐标), while `locationd` / developer location simulation expects WGS-84.
/// Injecting map-picked GCJ-02 as-is shifts the spoofed fix.
enum ChinaCoordinate {
    /// Broad China region including mainland, Taiwan, Hong Kong, and Macau.
    static func needsGCJCorrection(_ c: CLLocationCoordinate2D) -> Bool {
        let lat = c.latitude
        let lon = c.longitude
    ...
    /// Convert MapKit (GCJ-02) → WGS-84 for locationd injection.
    static func gcj02ToWgs84(_ c: CLLocationCoordinate2D) -> CLLocationCoordinate2D {
        guard needsGCJCorrection(c) else { return c }
