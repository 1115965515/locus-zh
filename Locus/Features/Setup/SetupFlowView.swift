...
struct SetupFlowView: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(supportsOnDevicePairing
                     ? "Locus 需要一次性配对才能设置位置。你将在设置中确认一个短码。"
                     : "请从电脑导入配对文件——Locus 用它在这台设备上安全地设置位置。")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var importPairingCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            stepRow(2, "通过隔空投送/分享到 Locus，或复制 plist 文本。")
            stepRow(3, "点「导入」；若选择器不可用（LiveContainer），可从剪贴板粘贴。")
        }
        .padding(18)
    ...
    private var vpnPage: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 22) {
                ...
                VStack(spacing: 10) {
                    Text(localDevVPNInstalled ? "连接 LocalDevVPN" : "再安装一个 App")
                        .font(.title.weight(.bold))

                    Text(localDevVPNInstalled
                         ? "已安装 LocalDevVPN。请打开它并启用 Locus 所需的私有隧道，然后返回这里。"
                         : "LocalDevVPN 会创建私有隧道，供 Locus 与手机定位系统通信。安装并开启后，即可开始传送。")
                        .font(.body)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    if localDevVPNInstalled {
                        tipRow(systemImage: "checkmark.circle.fill", title: "已安装", detail: "本机已有 LocalDevVPN。")
                        tipRow(systemImage: "power.circle.fill", title: "连接", detail: "点下方打开并启动隧道，随后会返回 Locus。")
                    } else {
                        tipRow(systemImage: "arrow.down.app.fill", title: "安装", detail: "从 App Store 获取 LocalDevVPN。")
                        tipRow(systemImage: "power.circle.fill", title: "连接", detail: "打开并启用 VPN，保持默认 IP 即可。")
                    }
                    tipRow(systemImage: "wifi", title: "首次传送请用 Wi‑Fi", detail: "请先在 Wi‑Fi 下完成第一次传送，之后蜂窝网络也可继续。")
                }
                .padding(18)
                .locusGlass(.regular, in: RoundedRectangle(cornerRadius: 20, style: .continuous))
            }
